function vm(files, stage, options)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Voxelwise modeling for predefined ROIs or whole brain
% --
...
[~, rspca, ~, ~, rexx] = pca(stim(:,2:end));
rnpca = 300; %number of principal components used
rspca = rspca(1:3870,1:rnpca);
sum(rexx(1:rnpca)) % explained by first npca components
stim = [stim(:,1),rspca]; %Append motion energy column
...
switch stage
    ...
    case {1}
        ...
        tccsall = zeros(cvfolds,ctfolds,length(as),length(targetvoxels));
        mccsall = zeros(cvfolds,ctfolds,length(as));
        ...
        for cvidx = 1:cvfolds
            ...
            for tidx = 1:ctfolds
                fprintf('%d of %d(ct)\n',tidx,ctfolds);
                if (length(sblock)<numtblock), sblock = trnblks{cvidx}; end % restart if sblock if done
                ...
                Ytest_pred = vm_solve(Xtrain, cat(2,Ytrain,nanmean(Ytrain,2)), as, Xtest);
                Ytest_pred = Ytest_pred(:,1:(end-1),:);                
                
                % Calculate prediction scores
                for asidx = 1:length(as)
                    % Calculate corr for all voxels
                    tccs = mvn_corr_fast(Ytest, Ytest_pred(:,:,asidx));
                    tccsall(cvidx,tidx,asidx,:) = tccs;
                    mccs = nanmean(tccs);
                    % Store all correlation values
                    mccsall(cvidx,tidx,asidx) = mccs;
                end
            end
        end
        ...
        % save the output
        eval(['save -v7.3 ' files.out(1:end-4) '_cv_lambda.mat as tccsall mccsall trainingSamples testingSamples;']);
        
        %-----------------------------------------------------------------
        % Plot prediction scores across reg. params to see optimal values
        %-----------------------------------------------------------------
        mccsall = reshape(mccsall, [cvfolds*ctfolds length(as)]);
        
        [cm, ci] = max(mean(mccsall,1));
        fprintf('Best reg. param based on corrs: %d\n', as(ci));
        
        figure;clf;
        semilogx(as, mean(mccsall,1));
